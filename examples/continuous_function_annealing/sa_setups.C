...
void setup_sa_func_comb(sa_cont_type &func_sa, int max_step, double t_max, double t_min,
  ...
  // give random initial guess
  func_sa.state_size=6;
  func_sa.state_curr=new double [func_sa.state_size];
  for(i=0; i<func_sa.state_size; i++){
    func_sa.state_curr[i]=(double)rand()/(double)RAND_MAX;
    func_sa.state_curr[i]=func_sa.state_curr[i]*2.0E+1-1.0E+1;
  }
  return;
}